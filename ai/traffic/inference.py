...
def track_video(source: Any, *, latitude: float | None = None, longitude: float | None = None,
    ...
    """Track vehicles through a video and emit periodic congestion events.

    This is a relative congestion estimator. It does not claim physical vehicle
    speed because a moving bus camera is not a calibrated speed sensor.
    """
    ...
    results = model.track(
        source=source,
        tracker=os.getenv("TIER2_TRACKER", "botsort.yaml"),
        classes=VEHICLE_CLASSES,
        conf=conf,
        iou=0.50,
        imgsz=imgsz,
        device=os.getenv("TIER2_DEVICE", "0"),
        vid_stride=stride,
        stream=True,
        verbose=False,
    )
    ...
    for frame_index, result in enumerate(results):
        ...
        if boxes is None or len(boxes) == 0:
            observations.append({"frame": frame_index, "vehicles": 0, "slow_ratio": 0.0, "occupancy": 0.0, "persistence": 0.0})
            continue
    ...
    densities = [o["vehicles"] for o in observations]
    max_density = max(densities) or 1
    ...
    for o in observations:
        ...
        events.append({
            "event_id": f"TRAFFIC-{o['frame']:08d}",
            "tier": 2,
